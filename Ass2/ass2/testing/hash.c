...
Word hash_any(char *k, int keylen)
{
	...
	switch (len)			/* all the case statements fall through */
	{
		case 11: c += ((Word) k[10] << 24); // fallthrough
		case 10: c += ((Word) k[9] << 16); // fallthrough
		case 9: c += ((Word) k[8] << 8); // fallthrough
			/* the lowest byte of c is reserved for the length */
		case 8: b += ((Word) k[7] << 24); // fallthrough
		case 7: b += ((Word) k[6] << 16); // fallthrough
		case 6: b += ((Word) k[5] << 8); // fallthrough
		case 5: b += k[4]; // fallthrough
		case 4: a += ((Word) k[3] << 24); // fallthrough
		case 3: a += ((Word) k[2] << 16); // fallthrough
		case 2: a += ((Word) k[1] << 8); // fallthrough
